...
sub usage {
        ...
        print "vdbench.pl -t [xiv|svc] -s [storage_name] -l [log_dir] \n";
        print "Example: vdbench -t svc -s rtcsvc17 -l 090415.11111\n";
        exit;
}
...
#---Get Param from XIV
        #
if ( not defined($storageType) && $storageType eq ""){
#if ( not defined($storageType) || not defined($Storage) || not defined($LogDir) ) {
    usage;
}
my ($xcli, $xiv_ver,$n_disks, $vdisk_size, $race_ver, @clients,$clients);
if ( lc($storageType) eq "xiv" ) {
...
} elsif ( lc($storageType eq "svc" ) ) {
    my $svc = "ssh -p 26 $Storage ";
}
exit(0);
